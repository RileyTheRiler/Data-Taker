...
function runOnceForDesktopLayout(testInfo) {
  test.skip(
    ...
    "Desktop layout coverage only needs one Chromium run."
  );
}
...
test("keeps desktop session actions near the cue controls and bounds the recent-trials sidebar", async ({ browser }, testInfo) => {
  runOnceForDesktopLayout(testInfo);
  const { context, page } = await openDesktopPage(browser);
  ...
  try {
    ...
    const desktopMedia = await page.evaluate(() => ({
      hover: matchMedia("(hover: hover)").matches,
      pointer: matchMedia("(pointer: fine)").matches,
    }));
    expect(desktopMedia).toEqual({ hover: true, pointer: true });
...
test("keeps the Objective draft directly under the desktop summary while graphs use the right column", async ({ browser }, testInfo) => {
  runOnceForDesktopLayout(testInfo);
  const { context, page } = await openDesktopPage(browser);
